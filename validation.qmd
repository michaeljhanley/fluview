...
box_plot <- ggplot(data) +
  geom_boxplot(aes(x = factor(epiweek), y = num_providers)) +
  labs(title = "Distribution of Providers by Week",
       x = "Epiweek",
       y = "Number of Providers") +
  theme_minimal() +
...
library(gridExtra)
grid.arrange(time_plot, box_plot, ncol = 1)
```
...
Misc. validation tasks:

[ ] String length violations (region has to be 5 or less characters)
[ ] Number range constraints (none of our numeric columns can be negative)
[ ] Cross-field validation, if applicable (num_ili has to equal sum of age 0 through 5)
[ ] Start and end date validation (Data can't be before week 40 of 2014, or after week 22 of 2019)
